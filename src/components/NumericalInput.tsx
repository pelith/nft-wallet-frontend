...
type ChakraInputProps = Parameters<ComponentWithAs<'input', InputProps>>[0]

const inputRegex = RegExp('^\\d*(?:\\\\[.])?\\d*$') // match escaped "." characters via in a non-capturing group

interface NumericalInputProps extends Omit<ChakraInputProps, 'onChange'> {
  value: string | number
...
const NumericalInput = React.forwardRef<HTMLInputElement, NumericalInputProps>(
  (
    { value, onUserInput, placeholder, maxLength, decimals = 18, setIsFocused, ...rest },
    ref,
  ) => {
